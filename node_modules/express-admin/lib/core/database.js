	...
	if (record.insert) {
		var data = kvp(args.name, record.columns, settings.columns),
			str = query.replace('insert',
				settings.table.name, data.keys.join(), data.values.join(), settings.table.pk);
...
function manyInsert (ids, link, record) {
	...
	var str = query.replace('insert-many',
		link.table, [sql.justName(link.parentPk), sql.justName(link.childPk)].join(),
		values.join());
